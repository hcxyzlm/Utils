...
//生成密文
string CMD5Encrypt::EncryptData(unsigned char *pszSrcData)
{
	//加密密文
	CMD5 MD5Encrypt;
	unsigned char szResult[16];
	char szMD5Result[64];
	int Len = strlen((char*)pszSrcData);
	MD5Encrypt.MD5Update((unsigned char *)pszSrcData,Len*sizeof(char));
	MD5Encrypt.MD5Final(szResult);

	memset(szMD5Result, 0, 64);
	for (int i=0;i<16;i++) 
